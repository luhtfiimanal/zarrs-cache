...
mod s3_tests {
    ...
    fn get_test_config() -> (String, String, String) {
        ...
        let zarr_path =
            env::var("S3_ZARR_PATH").unwrap_or_else(|_| "satellite_data.zarr".to_string());
        let chunk_prefix =
    ...
    async fn test_satellite_zarr_cache_performance() {
        ...
        let (bucket, zarr_path, chunk_prefix) = get_test_config();
        ...
        let test_chunks = vec![
            format!("{}/1/1/1", chunk_prefix),
            format!("{}/1/1/2", chunk_prefix),
            format!("{}/1/2/1", chunk_prefix),
            format!("{}/2/1/1", chunk_prefix),
        ];
    ...
    async fn test_hybrid_cache_with_satellite_data() {
        ...
        for x in 1..=5 {
            for y in 1..=5 {
                for z in 1..=3 {
                    test_chunks.push(format!("{}/{}/{}/{}", chunk_prefix, x, y, z));
                }
            }
        }
        ...
        for (i, chunk_key) in test_chunks.iter().enumerate() {
            ...
            if i % 10 == 0 {
                let stats = store.cache_stats();
                println!(
                    "📊 Loaded {} chunks, cache stats: hits={}, misses={}",
                    i + 1,
                    stats.hits,
                    stats.misses
                );
        ...
        println!(
            "📈 Final stats: hits={}, misses={}, hit_rate={:.2}%",
            final_stats.hits,
            final_stats.misses,
